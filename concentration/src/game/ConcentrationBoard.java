...
public class ConcentrationBoard {
    ...
    public ConcentrationCard getCard(int row, int col) throws ConcentrationException {
        if(row>=DIM || col>=DIM){
            throw new ConcentrationException(String.format(ConcentrationProtocol.ERROR, " Invalid coordinates"));
        }
    ...
    public CardMatch reveal(int row, int col) throws ConcentrationException {
        ...
        ConcentrationCard currentCard = getCard(row, col);
        if (revealedCard == null){
            revealedCard = currentCard;
            cardMatch = new CardMatch(revealedCard, null, false);

        } else {
            boolean isMatch = currentCard.equals(revealedCard);
            if(isMatch){
                matches++;
            }
            cardMatch = new CardMatch(revealedCard, currentCard, isMatch);
            revealedCard = null;
        }
        return cardMatch;
    }
